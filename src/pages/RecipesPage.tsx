...
import { Plus, ChefHat, ChevronRight } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between px-1">
        ...
        <motion.button onClick={() => setAddOpen(true)} whileTap={tap} className="flex items-center gap-1 text-sm text-fg-muted">
          <Plus size={15} />
          New recipe
        </motion.button>
      </div>
      ...
      {recipes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-fg-faint text-center">
            Create a recipe with its ingredients and steps, then link it to a cooking
            task or use it when logging a meal.
          </p>
        </div>
      ) : (
        recipes.map((recipe) => (
          ...
          >
            <div
              ...
              style={{ backgroundColor: recipe.color, color: isLightColor(recipe.color) ? '#111827' : '#fff' }}
            >
              <ChefHat size={20} />
            </div>

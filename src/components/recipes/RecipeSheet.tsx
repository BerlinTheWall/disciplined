...
const COLOR_OPTIONS = [
  ...
  '#f472b6', '#22d3ee', '#a3e635', '#fb7185', '#f87171',
]
          ...
          >
            ...
            <div className="px-4 pt-3 pb-5 rounded-t-2xl" style={{ backgroundColor: color }}>
              <div className="flex items-center justify-between">
                ...
                {isEditing && (
                  <motion.button onClick={handleDelete} whileTap={tap} className="w-9 h-9 rounded-full flex items-center justify-center" style={headerBtn}>
                    <Trash2 size={18} />
                  </motion.button>
                )}
              </div>
            ...
            <div className="p-4 pb-6">
              ...
              <div className="flex gap-3 mb-5">
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between bg-surface-raised rounded-2xl px-3 py-2.5">
                    <motion.button onClick={() => setServings((s) => Math.max(1, s - 1))} whileTap={tap} className="w-7 h-7 rounded-full bg-surface text-fg-muted flex items-center justify-center">
                      <Minus size={14} />
                    </motion.button>
                    <span className="text-base font-semibold text-fg tabular-nums">{servings}</span>
                    <motion.button onClick={() => setServings((s) => s + 1)} whileTap={tap} className="w-7 h-7 rounded-full bg-surface text-fg-muted flex items-center justify-center">
                      <Plus size={14} />
                    </motion.button>
                  </div>
                </div>
              ...
              <div className="flex gap-3 overflow-x-auto bg-surface-raised rounded-full p-1.5 mb-6" style={{ scrollbarWidth: 'none' }}>
                {COLOR_OPTIONS.map((c) => (
                  <motion.button
                    key={c}
                    onClick={() => setColor(c)}
              ...
              {groceryItems.length === 0 ? (
                <p className="text-sm text-fg-faint mb-5">
                  Add items in the Food &amp; Products section first — recipes are built from the same catalog.
                </p>
              ) : (
                <div className="flex flex-col gap-2 mb-3">
                  {groceryItems.map((item) => {
                    ...
                    return (
                      <div key={item.id} className={`flex items-center gap-3 p-2.5 pr-3 rounded-2xl ${selected ? 'bg-surface-inverse' : 'bg-surface-alt'}`}>
                        <motion.button onClick={() => toggleItem(item.id)} whileTap={tap} className="flex items-center gap-2.5 flex-1 min-w-0 text-left">
                          <span className="w-7 h-7 rounded-full flex items-center justify-center text-white shrink-0" style={{ backgroundColor: cat.color }}>
                            {selected ? <Check size={15} strokeWidth={3} /> : <Icon size={14} />}
                          </span>
                          <span className="min-w-0">
                            <span className={`block font-medium leading-tight truncate ${selected ? 'text-fg-inverse' : 'text-fg'}`}>{item.name}</span>
                            <span className={`block text-xs mt-0.5 ${selected ? 'text-fg-muted-inverse' : 'text-fg-faint'}`}>
                              {selected
                        ...
                        {selected && (
                          <div className="flex items-center gap-1.5 shrink-0">
                            <motion.button onClick={() => bumpServings(item.id, -0.25)} whileTap={tap} className="w-7 h-7 rounded-full bg-surface-raised text-fg-muted flex items-center justify-center" aria-label="Less">
                              <Minus size={14} />
                            </motion.button>
                            <span className="w-9 text-center text-sm font-medium text-fg-inverse">×{srv}</span>
                            <motion.button onClick={() => bumpServings(item.id, 0.25)} whileTap={tap} className="w-7 h-7 rounded-full bg-surface-raised text-fg-muted flex items-center justify-center" aria-label="More">
                              <Plus size={14} />
                            </motion.button>
                          </div>
                        )}
              ...
              <div className="flex flex-col gap-2">
                <AnimatePresence initial={false}>
                  {steps.map((step, i) => (
                    ...
                    >
                      <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 mt-0.5" style={{ backgroundColor: color, color: onColor }}>
                        {i + 1}
                      </span>
                      <textarea
                        value={step}
                      ...
                      />
                      <motion.button onClick={() => removeStep(i)} whileTap={tap} className="w-7 h-7 rounded-full text-fg-faint hover:text-fg flex items-center justify-center shrink-0" aria-label="Remove step">
                        <X size={16} />
                      </motion.button>
                    </motion.div>
                  ))}
                </AnimatePresence>
                <motion.button onClick={addStep} whileTap={tap} className="flex items-center justify-center gap-1.5 rounded-2xl border border-dashed border-border-strong py-3 text-sm font-medium text-fg-muted">
                  <Plus size={16} />
                  Add step
                </motion.button>
              </div>

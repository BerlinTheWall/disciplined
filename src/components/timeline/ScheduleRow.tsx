...
import { minutesToPx, pxToMinutes, snapToGrid, formatTimeLabel, formatTimeRange, getPillHeight, PILL_BASE_SIZE } from '../../lib/time'
...
export default function ScheduleRow({ id, title, startMinutes, durationMinutes, color, icon, completed, streak, onToggle }: ScheduleRowProps) {
  ...
  return (
    <div
      ...
      style={{ top: minutesToPx(startMinutes + liveOffsetMinutes), height: rowHeight, opacity: isActive ? 0.9 : 1, zIndex: isActive ? 10 : 1 }}
    >
      <div className="w-12 pt-2 text-xs text-gray-400 text-right shrink-0">{formatTimeLabel(startMinutes)}</div>
      ...
      <div
        ...
        style={{ width: PILL_BASE_SIZE, height: pillHeight, backgroundColor: color }}
      >
      ...
      <div className="flex-1 pt-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className={`font-semibold text-gray-900 leading-tight ${completed ? 'line-through text-gray-400' : ''}`}>{title}</p>
        ...
        <p className="text-xs text-gray-400 mt-0.5">{formatTimeRange(startMinutes, durationMinutes)}</p>

        <div
          ref={resize.setNodeRef}
          {...resize.attributes}
          onPointerDown={(e) => { e.stopPropagation(); resize.listeners?.onPointerDown?.(e) }}

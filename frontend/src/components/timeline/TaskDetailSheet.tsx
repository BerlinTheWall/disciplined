...
export default function TaskDetailSheet({
  ...
  const [presets, addPreset, removePreset] = usePresetStore(
    useShallow((s) => [s.presets, s.addPreset, s.removePreset])
  );
  ...
  // Presets are unique by title (see presetStore), so that's how an item maps
  // to "already saved as a preset" — which fills the star. Tasks and habits
  // can both be starred; either becomes a one-tap *task* preset (a habit's
  // has no priority, since habits don't carry one).
  // Below Plus the star still shows (so the feature is discoverable) but never
  // reads as saved, and tapping it explains the plan instead.
  const canUsePresets = useHasTier(PRESETS_MIN_TIER);
  const savedPreset =
    data && canUsePresets
      ? presets.find((p) => p.title.trim().toLowerCase() === data.title.trim().toLowerCase())
      : undefined;

  async function togglePreset() {
    if (!data) return;
    if (!canUsePresets) {
      await confirm(PRESETS_LOCKED_DIALOG);
      return;
    }
    if (savedPreset) {
      removePreset(savedPreset.id);
      useToastStore.getState().show("Removed from presets");
      return;
    }
    if (presets.length >= MAX_PRESETS) {
      const manage = await confirm({
        title: `You already have ${MAX_PRESETS} presets`,
        message: `That's the most you can keep. Remove one to make space for "${data.title}".`,
        confirmLabel: "Manage presets",
        cancelLabel: "Not now",
      });
      if (manage) setShowPresets(true);
      return;
    }
    addPreset({
      title: data.title,
    ...
    useToastStore.getState().show("Saved as a preset");
  }
  ...
  return (
    <>
            ...
            <div className="px-4 pt-3 pb-5 rounded-t-2xl" style={{ backgroundColor: color }}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/* Save / unsave this task or habit as a one-tap preset. */}
                  <motion.button
                    onClick={() => void togglePreset()}
                    whileTap={tap}
                    aria-label={savedPreset ? "Remove from presets" : "Save as preset"}
                    aria-pressed={!!savedPreset}
                    className="w-9 h-9 rounded-full flex items-center justify-center"
                    style={{ backgroundColor: headerBtnBg, color: onColor }}
                  >
                    <AnimatePresence mode="wait" initial={false}>
                      <motion.span
                        key={savedPreset ? "on" : "off"}
                        ...
                        className="flex"
                      >
                        <Star size={17} fill={savedPreset ? "currentColor" : "none"} />
                      </motion.span>
                    </AnimatePresence>
                  </motion.button>
                </div>
            ...
            <div className="p-4 pb-8 flex flex-col gap-2">
              {onShowOnCalendar && (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0 text-sm font-medium text-fg">
                    Show on calendar
                  </span>
      ...
      {/* Opened from the preset-limit popup — stacks above this sheet so the
        user can make room, then star the task again. */}
      <PresetsSheet elevated isOpen={showPresets && !!item} onClose={() => setShowPresets(false)} />
    </>
  );
}

...
export default function GoalScheduleSheet({ goal }: { goal: Goal }) {
  ...
  return (
    <>
                        ...
                        >
                          ...
                          {resolved ? (
                            ...
                            <motion.button
                              ...
                              whileTap={tap}
                              aria-label={`Change time for ${title}`}
                              className="text-[12px] text-fg-faint shrink-0"
                            >
                              {date && `${formatShortDate(date)} · `}
                              {start != null ? rangeLabel(start, duration) : "auto-picked time"}
                            </motion.button>
      ...
      >
        <TimeWheel
          value={formatTimeLabel(editingStart ?? 9 * 60)}
          color={accent}
          onChange={(next) => {
            if (editingIndex != null) setProposalTime(editingIndex, timeStringToMinutes(next));
          }}
          visibleRows={5}
        />
      </FieldPanel>

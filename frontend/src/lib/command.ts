...
function parseTarget(text: string, now: Date): TargetHint {
  ...
  const titleWords = rest
    .replace(FILLER, " ")
    .replace(
      /\b(?:tonight|today|tomorrow|morning|afternoon|evening|night|noon|midday|midnight|pm|am)\b/g,
      " "
    )
    .replace(/[^a-z0-9\s]/g, " ")
    .split(/\s+/)
...
export function parseCommand(input: string, now: Date = new Date()): Command | null {
  ...
  if (habitCue && (convertVerb || /\brepeat\b/.test(text))) {
    ...
    const targetText = text
      .replace(
        /\b(?:make|turn|convert|change|set|repeat(?:s|ing|ed)?|recurring|daily|every)\b/g,
        " "
      )
      .replace(
        /\b(?:everyday|weekdays?|weekends?|sun|sunday|mon|monday|tue|tues|tuesday|wed|weds|wednesday|thu|thur|thurs|thursday|fri|friday|sat|saturday|day|days|week)\b/g,
        " "
      );
  ...
  if (uncheck) {
    const t = text.replace(
      /\b(?:mark|uncheck|uncomplete|incomplete|undone|not|done|complete|finished|as)\b/g,
      " "
    );
  ...
  if (check) {
    const t = text.replace(
      /\b(?:mark|check|tick|cross|off|done|complete|completed|finish|finished|as)\b/g,
      " "
    );
  ...
  if (durValue != null && (addDur || subDur) && /\bby\b/.test(text)) {
    const t = text.replace(
      /\b(?:add|extend|increase|lengthen|shorten|reduce|cut|decrease|trim|by)\b/g,
      " "
    );
    return {
      action: "duration",
      target: parseTarget(stripDuration(t), now),
      deltaMinutes: subDur ? -durValue : durValue,
    };
  }
  ...
  if (/\bduration\b/.test(text)) {
    ...
    if (n) {
      const t = text
        .replace(/\b(?:set|change|duration|to|make|it|mins?|minutes?)\b/g, " ")
        .replace(/\b\d+\b/, " ");
